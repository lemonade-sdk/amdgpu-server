...
// Minimal base64 decoder for image data URLs.
std::string base64_decode(const std::string& input) {
    ...
    for (unsigned char c : input) {
        if (T[c] == -1) continue;  // skip whitespace/newlines/padding
        val = (val << 6) + T[c];
        valb += 6;
        if (valb >= 0) {
...
// Resolve a single image reference (data URL, local file path) to raw bytes.
// Returns empty string if it cannot be resolved.
std::string resolve_image_bytes(const std::string& url) {
    const std::string data_prefix = "data:";
    if (url.rfind(data_prefix, 0) == 0) {
        size_t comma = url.find(',');
        if (comma == std::string::npos) return "";
        bool is_b64 = url.substr(0, comma).find(";base64") != std::string::npos;
        std::string payload = url.substr(comma + 1);
        return is_b64 ? base64_decode(payload) : payload;
    }
    // Local file path fallback (file:// or bare path)
    std::string path = url;
    const std::string file_prefix = "file:///";
    if (path.rfind("file:///", 0) == 0) path = path.substr(file_prefix.size());
    else if (path.rfind("file://", 0) == 0) path = path.substr(std::string("file://").size());
...
// Extract image bytes from OpenAI-style messages with array content
// (items of {"type":"image_url","image_url":{"url":...}} or {"image_url":...}).
std::vector<std::string> extract_request_images(const json& request_json) {
    ...
    for (const auto& msg : request_json["messages"]) {
        ...
        for (const auto& item : msg["content"]) {
            ...
            if (!bytes.empty()) {
                ...
                std::cerr << "[WARNING] Could not resolve image reference (skipped)" << std::endl;
            }
        }
    }
    return images;
...
void RyzenAIServer::handleChatCompletions(const httplib::Request& req, httplib::Response& res) {
    try {
        ...
        // messages so structured (array) content survives for the template.
...
void RyzenAIServer::handleMultimodalChat(const json& request_json,
    ...
    const std::string model_id = model_id_;
            ...
            [this, prompt, images, params, model_id, prompt_tokens, escapeJson](size_t offset, httplib::DataSink& sink) {
                ...
                try {
                    ...
                    inference_engine_->streamCompleteMultimodal(prompt, images, params,
                        [&sink, model_id, &token_count, &escapeJson](const std::string& token, bool /*is_final*/) -> bool {
                            std::string chunk =
                                ...
                                "\",\"object\":\"chat.completion.chunk\",\"created\":" + std::to_string(std::time(nullptr)) +
                                ",\"model\":\"" + model_id +
                                "\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"" + escapeJson(token) +
                                "\"},\"finish_reason\":null}]}\n\n";
                            token_count++;
                    ...
                    std::string final_chunk =
                        ...
                        ",\"completion_tokens\":" + std::to_string(token_count) +
                        ",\"total_tokens\":" + std::to_string(prompt_tokens + token_count) + "}}\n\n";
                    sink.write(final_chunk.c_str(), final_chunk.size());
    ...
    json response = {
        ...
        {"usage", {
            ...
            {"total_tokens", prompt_tokens + timing.token_count},
            {"prefill_duration_ttft", timing.ttft_seconds},
            {"decoding_speed_tps", timing.tps}
        }}
    };
